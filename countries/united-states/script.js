(function () {
  ...
  document.getElementById('saveBtn').addEventListener('click', function () {
    ...
    showToast(nowSaved ? 'Country profile saved' : 'Removed from saved profiles');
  });
  ...
  document.getElementById('shareBtn').addEventListener('click', async () => {
    ...
    if (navigator.share) {
      try {
        await navigator.share({ title: 'United States — Country Profile', url });
        return;
      } catch (err) {
  ...
  document.getElementById('loadMoreFounders').addEventListener('click', () => {
